...
from chemprop.models.MoleculeModel_Multiple import MoleculeModel_Multiple
from chemprop.models.MoleculeModel_Multiple_intermediate import MoleculeModel_Multiple_intermediate
from chemprop.nn_utils import param_count, param_count_all
from chemprop.utils import build_optimizer, build_lr_scheduler, load_checkpoint, load_checkpoint_multi, load_checkpoint_multi_intermediate,makedirs, \
    save_checkpoint,save_checkpoint_multi,save_checkpoint_multi_intermediate, save_smiles_splits, load_frzn_model, multitask_mean,multitask_mean_updated,load_encoder_model
import ast


def run_training_updated(args: TrainArgs,
    ...
    for model_idx in range(args.ensemble_size):
        ...
        # Load/build model
        if args.multi_modality_ensemble == 'True':
            if args.checkpoint_paths is not None:
                debug(f'Loading Ensemble model from {args.checkpoint_paths}')
                model = load_checkpoint_multiple(args.checkpoint_paths, logger=logger)
            else:
                debug(f'Building Ensemble model')
                #model = MoleculeModel_Multiple(args,5)
                list_of_model = ast.literal_eval(args.encoder_path)
                num_models = len(list_of_model)
                model = MoleculeModel_Multiple(args,num_models)
            ...
            if args.checkpoint_paths is not None:
                ...
                list_of_model = ast.literal_eval(args.encoder_path)
                num_models = len(list_of_model)
                model = MoleculeModel_Multiple_intermediate(args,num_models)
        elif args.multi_modality_ensemble == 'False':
            if args.checkpoint_paths is not None:
                ...
                if args.encoder_path is not None:
                    model = load_encoder_model(model=model,path=args.encoder_path,current_args=args, logger=logger)
                #if args.encoder_path is not None:

                # Load the encoder after contrastive learning "
    #             debug(f'Loading encoder for model {model_idx}')
    #             model.encoder.load_state_dict(torch.load("/home/zhengyjo/chemprop-master/M3_KMGCL_encoder_smiles.pt"))
        ...
        if args.multi_modality_ensemble == 'False':
            ...
            save_checkpoint_multi(os.path.join(save_dir, MODEL_FILE_NAME), model, scaler,
                            features_scaler, atom_descriptor_scaler, bond_descriptor_scaler,
                            atom_bond_scaler, args)
        ...
        for epoch in trange(args.epochs):
            ...
            for metric, scores in val_scores.items():
                ...
                mean_val_score = multitask_mean_updated(scores, metric=metric)
...
#                 best_score, best_epoch = mean_val_score, epoch
        ...
        if args.multi_modality_ensemble == 'False':
            ...
            model = load_checkpoint_multi(os.path.join(save_dir, MODEL_FILE_NAME), device=args.device, logger=logger)
        elif args.multi_modality_ensemble == 'Intermediate':
            model = load_checkpoint_multi_intermediate(os.path.join(save_dir, MODEL_FILE_NAME), device=args.device, logger=logger)
    ...
    for metric, scores in ensemble_scores.items():
        ...
        mean_ensemble_test_score = multitask_mean_updated(scores, metric=metric)
